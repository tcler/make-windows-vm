...
run() {
	local _runtype= _debug=yes _rc=0
	local _nohup= _nohuplogf=
	local _defaultlogf=nohup.log
	...
	local _cmdl=$(getSafeCommandLine "$@")

	if [[ "$_debug" = yes ]]; then
		[[ "$_nohup" = yes ]] && _cmdl="nohup $_cmdl &>${_nohuplogf} &"
		echo "[${_runtype:-direct} run]" "$_cmdl"
	fi | GREP_COLORS='ms=01;30;47' grep --color .

	case ${_runtype:-direct} in
	direct) if [[ -n "$_nohup" ]]; then nohup "$@" &>${_nohuplogf} & else "$@"; _rc=$?; fi;;
	eval)   eval "$_cmdl"; _rc=$?;;
	bash)   bash -c "$_cmdl"; _rc=$?;;
	esac
...
Examples:
  ...
  ./make-win-vm.sh --image /var/lib/libvirt/images/Win2019-Evaluation.iso \
    --os-variant win2k19 --vmname win2019-cifsnfs --domain cifs-nfs.test -p ~Ocgxyz \
